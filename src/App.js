...
import {
  BrowserRouter as Router,Routes,Route,Link,useNavigate,useParams,
} from "react-router-dom";
import styled, { createGlobalStyle } from "styled-components";
import {
  FaUserCircle,FaFutbol,FaCalendarAlt,FaClock,FaRupeeSign,FaMapMarkerAlt,FaStar,
} from "react-icons/fa";
...
const TurfProvider = ({ children }) => {
  ...
  const updateBookingStatus = (bookingId, newStatus, paymentStatus = null) => {
    setBookings((prevBookings) =>
      prevBookings.map((booking) =>
        booking.id === bookingId
          ? { ...booking, status: newStatus, ...(paymentStatus && { paymentStatus }) }
          : booking
      )
    );
...
const Header = () => {
  ...
  const visibleUsers = currentUser?.role === "player"
    ...
    <HeaderContainer>
      ...
      {currentUser ? (
        <UserSection>
          ...
          {showRoleToggle && currentUser?.role === "admin" && (
            <RoleToggle>
              {users.map((user) => (
                <RoleOption
                  key={user.id}
...
const TurfDetailPage = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { turfs, bookings, reviews, addBooking, addReview, updateBookingStatus, currentUser, users } = useContext(TurfContext);
  ...
  const handleBookSlot = () => {
    if (turf && selectedDate && selectedTimeSlot && currentUser.role === "player") {
      ...
      toast.error("Admins cannot book slots as players from this interface. Use the admin dashboard to block slots.");
    } else {
  ...
  const initiatePayment = () => {
    ...
    script.onload = () => {
      const options = {
        ...
        amount: turf.pricePerHour * 100, // Convert to paise
        currency: "INR",
        name: "JustPlay",
        description: `Booking for ${turf.name}`,
        image: "https://your-logo-url.com/logo.png", // Replace with your logo URL
        handler: function (response) {
          updateBookingStatus(newBookingId, "confirmed", "paid");
      ...
      paymentObject.on("payment.failed", function (response) {
        updateBookingStatus(newBookingId, "rejected", "failed");
  ...
  return (
    <TurfDetailContainer>
      ...
      <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)}>
        ...
        <p>Please complete the payment through the Razorpay interface.</p>
        <Button
...
const AdminDashboardPage = () => {
  ...
  const handleBlockSubmit = (e) => {
    ...
    if (blockFormData.turfId && blockFormData.date && blockFormData.timeSlot) {
      ...
      toast.error("Please fill all fields to block a slot.");
    }
  };
